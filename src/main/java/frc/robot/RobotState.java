...
public class RobotState {
   ...
   public boolean getIsIntaking(){
    ...
    public boolean isInWing() {
        ...
        if(isRedAlliance()){
            inWing = robotPose.getX() > Constants.FieldConstants.RED_WING_LINE;
        } else {
            inWing = robotPose.getX() < Constants.FieldConstants.BLUE_WING_LINE;
        }
    ...
    public Translation2d getSpeakerLocation() {
        if(isRedAlliance()) {
            return Constants.FieldConstants.RED_SPEAKER_LOCATION;
        } else {
            return Constants.FieldConstants.BLUE_SPEAKER_LOCATION;
        }
    }

    public Translation2d getSpeakerAimLocation() {
        if(isRedAlliance()) {
            return Constants.FieldConstants.RED_SPEAKER_AIM_LOCATION;
        } else {
            return Constants.FieldConstants.BLUE_SPEAKER_AIM_LOCATION;
        }
    }
